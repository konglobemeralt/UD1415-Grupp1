...
void D3D::Update()
{
	...
	{
		ReadMemory(smType);
		meshes.back().meshesBuffer[0] = CreateMesh(sizeof(XMFLOAT3) * meshes.back().vertexCount, meshes.back().pos.data(), meshes.back().vertexCount);
		meshes.back().meshesBuffer[1] = CreateMesh(sizeof(XMFLOAT2) * meshes.back().vertexCount, meshes.back().uv.data(), meshes.back().vertexCount);
		meshes.back().meshesBuffer[2] = CreateMesh(sizeof(XMFLOAT3) * meshes.back().vertexCount, meshes.back().normal.data(), meshes.back().vertexCount);
...
void D3D::Render()
{
	...
	for (size_t i = 0; i < meshes.size(); i++)
	{
		if (meshes[i].meshesBuffer[0] != NULL)
		{
			...
			if (meshes[i].meshTex.textureExist.x == 1)
